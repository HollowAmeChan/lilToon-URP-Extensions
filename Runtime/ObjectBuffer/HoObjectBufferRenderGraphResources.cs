...
namespace lilToon.URP.Extensions.ObjectBuffer
{
    ...
    /// <c>frameData.GetOrCreate&lt;HoObjectBufferRenderGraphResources&gt;()</c> 后按需 <c>UseTexture(..., AccessFlags.Read)</c>。
    /// <para>
    /// **CB 只有身份与覆盖率**（决策 20）：<c>HasRequiredTextures</c> 要求 ID 层与覆盖率，
    /// 选择层按需。表面色与材质数值在 `Ho-SurfaceBuffer`，不在这里。
    /// </para>
    /// </summary>
    internal sealed class HoObjectBufferRenderGraphResources : ContextItem
    {
