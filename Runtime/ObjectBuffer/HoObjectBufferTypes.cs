...
namespace lilToon.URP.Extensions.ObjectBuffer
{
    ...
    /// <summary>
    /// 组件级的赋值方式：一个物体被多个部件条目命中时，它归谁。**默认「覆盖」**。
    /// <list type="bullet">
    /// <item>像素里的身份是 16 bit `组:8 | 槽位:8`，一个 renderer 只可能有一个身份，所以"重叠"本身没法表达 ——
    /// 这一项选的不是"要不要允许重叠"，而是**用哪种显式规则决定归谁**。</item>
    /// <item><b>覆盖</b>：默认。**条目顺序即优先级，排在下面的条目接管上面条目里的同一个物体。**
    /// 于是"顶上放一条『全体』（全角色），下面放人体 / 脸 / 前发（各自的位）"直接可写，
    /// 不需要反选没归属的物体、也不用给每个部件重复勾位；同组内不再报冲突。</item>
    /// <item><b>指定</b>：审计模式。重叠算配置错误，面板底部逐条列出谁赢谁输，同组内取条目顺序在前者。
    /// 想确认"到底哪些物体被下面的条目接管了"时切回来看一眼即可。</item>
    /// <item><b>两种模式都不继承标签</b>（刻意）：palette 一行 = 一个部件 = 一份标签掩码，像素里只有 `partId`；
    /// 若做继承，同一部件里"被接管的物体"与"没被接管的物体"就需要两份标签、一行装不下。
    /// 所以条目的标签就是它拿到的那些物体的全部标签 —— 想让被接管的物体同时保住上面那条的位，就在下面那条里一起勾。</item>
    /// <item>跨组仍是"离 Renderer 更近的组胜、距离相同用组 ID 定序"，且一律报冲突（两个组抢同一个 renderer 是真的配置问题）。</item>
    /// </list>
    /// </summary>
