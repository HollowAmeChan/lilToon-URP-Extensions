Shader "Hidden/lilToon-Shoost/URP/Shoost/VHS"
{
    SubShader
    {
        Tags
        ...
        Pass
        {
            ...
            float ShoostHash(float2 p)
            {
                return frac(sin(dot(p, float2(89.44, 19.36))) * 22189.220703);
            }

            float ShoostValueNoise(float2 p)
            {
                float2 grid = p * 4.0;
                ...
                float a = ShoostHash(cell);
                float b = ShoostHash(cell + float2(0.25, 0.0));
                float c = ShoostHash(cell + float2(0.0, 0.25));
                float d = ShoostHash(cell + float2(0.25, 0.25));
            ...
            {
                float time = _Time.y;
                float firstWave = ShoostValueNoise(float2(uv.y + 1.0, time + 1.0));
                float secondWave = ShoostValueNoise(float2(uv.y * 100.0 + 1.0, time * 10.0 + 1.0));
                ...
                float tapeNoise = ShoostValueNoise(float2(uv.y * 100.0 + 1.0, time * 5.0 + 1.0));
                tapeLine = min(max((tapePattern - 0.92) * tapeNoise * 0.5, 0.0), 0.01) * 10.0;

                float tapeJitter = max(ShoostValueNoise(float2(uv.y * 100.0 + 13.0, time * 5.0 + 7.0)) - 0.5, 0.0);
            ...
            float3 GetNoise2Color(float2 uv, float3 warpedColor, float type)
            {
                ...
                float lineNoise = ShoostValueNoise(float2(uv.y * 100.0 + 1.0, _Time.y * 5.0 + 1.0));
