    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half3 ApplyLayerBlend(half3 baseColor, half3 layerColor, float blendMode)
            {
                ...
                if (mode == 5)
                {
                    return ColorBurn(baseColor, layerColor);
                }
                ...
                if (mode == 8)
                {
                    return ColorDodge(baseColor, layerColor);
                }

                if (mode == 9)
                {
                    return Overlay(baseColor, layerColor);
                }

                if (mode == 10)
                {
                    return SoftLight(baseColor, layerColor);
                }

                if (mode == 11)
                {
                    return Overlay(layerColor, baseColor);
                }

                if (mode == 12)
                {
                    return VividLight(baseColor, layerColor);
                }
                ...
                if (mode == 14)
                {
                    return lerp(min(baseColor, 2.0 * layerColor), max(baseColor, 2.0 * (layerColor - 0.5)), step(0.5, layerColor));
                }

                if (mode == 15)
                {
                    return step(0.5, VividLight(baseColor, layerColor));
                }

                if (mode == 16)
                {
                    return abs(baseColor - layerColor);
                }

                if (mode == 17)
                {
                    return baseColor + layerColor - 2.0 * baseColor * layerColor;
                }
                ...
                if (mode == 20)
                {
                    return SetLum(SetSat(layerColor, Sat(baseColor)), Lum(baseColor));
                }

                if (mode == 21)
                {
                    return SetLum(SetSat(baseColor, Sat(layerColor)), Lum(baseColor));
                }

                if (mode == 22)
                {
                    return SetLum(layerColor, Lum(baseColor));
                }

                if (mode == 23)
                {
                    return SetLum(baseColor, Lum(layerColor));
                }
