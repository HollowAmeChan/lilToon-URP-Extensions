Shader "Hidden/lilToon-Shoost/URP/Shoost/BokehZoomBlur"
{
    SubShader
    {
        Tags
        ...
        Pass
        {
            ...
            static const float ShoostBokehZoomTwoPi = 6.28318530718;
            ...
            float ShoostBokehZoomLuma(float3 color)
            {
                return dot(color, float3(0.2126, 0.7152, 0.0722));
            }

            float ShoostBokehZoomHash(float2 p)
            {
            ...
            float ShoostBokehZoomHighlightWeight(float3 color, float threshold, float softKnee, float exposure)
            {
                float luma = ShoostBokehZoomLuma(max(color * max(exposure, 0.0), 0.0));
                if (softKnee <= 0.0001)
                {
            ...
            float ShoostBokehZoomAperture(float2 direction, float blades, float curvature, float rotationRadians)
            {
                ...
                float sector = ShoostBokehZoomTwoPi / bladeCount;
            ...
            float3 ShoostBokehZoomSampleRgb(float2 uv, float2 radialOffset, float chromatic)
            {
            ...
            float3 ShoostBokehZoomLayer(float2 uv)
            {
                ...
                float aperture = ShoostBokehZoomAperture(aspectDirection, _LayerParams2.x, _LayerParams2.y, radians(_LayerParams2.z));
                ...
                float jitter = (ShoostBokehZoomHash(floor(uv * _ScreenParams.xy)) - 0.5) / max(sampleCount, 1);
                float3 sum = 0.0;
                ...
                for (int i = 0; i < MaxBokehZoomSamples; i++)
                {
                    ...
                    float3 sampleColor = ShoostBokehZoomSampleRgb(sampleUV, offset, chromatic);
                    float highlightWeight = ShoostBokehZoomHighlightWeight(sampleColor, threshold, softKnee, exposure);
            ...
            float3 ShoostBokehZoomComposite(float3 source, float3 bokeh, float amount, float mode)
            {
            ...
            half4 FragBokehZoomBlur(Varyings input) : SV_Target
            {
                ...
                float3 bokeh = ShoostBokehZoomLayer(input.texcoord);
                if (_LayerParams3.y > 0.5)
                {
                    return half4(bokeh * amount, source.a);
                }

                return half4(ShoostBokehZoomComposite(source.rgb, bokeh, amount, _LayerParams3.x), source.a);
            }
            ENDHLSL
        }
    }
