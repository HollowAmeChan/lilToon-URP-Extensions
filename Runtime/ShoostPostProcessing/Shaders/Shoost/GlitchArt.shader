Shader "Hidden/lilToon-Shoost/URP/Shoost/GlitchArt"
{
    SubShader
    {
        Tags
        ...
        Pass
        {
            ...
            float ShoostGlitchHash11(float value)
            {
                return frac(sin(value * 127.1) * 43758.5453123);
            }

            float ShoostGlitchHash21(float2 value)
            {
                return frac(sin(dot(value, float2(127.1, 311.7))) * 43758.5453123);
            }

            half3 ShoostGlitchSampleRgb(float2 uv, float2 dispersion, float chromatic)
            {
            ...
            half4 FragGlitchArt(Varyings input) : SV_Target
            {
                ...
                float burst = smoothstep(0.35, 1.0, ShoostGlitchHash11(slowFrame * 2.13 + 19.0));
                float globalShake = (ShoostGlitchHash11(frame + 3.17) - 0.5) * shakeAmount * lerp(0.055, 0.135, burst);
                float verticalGate = step(1.0 - flicker * 0.65, ShoostGlitchHash11(slowFrame * 0.37 + 41.0));
                float verticalJump = (ShoostGlitchHash11(slowFrame + 17.0) - 0.5) * flicker * verticalGate * 0.10;

                float lineIndex = floor(uv.y * stripeDensity);
                float lineRnd = ShoostGlitchHash11(lineIndex + frame * 1.31);
                float lineGate = step(1.0 - scanlineJitter * 0.72, ShoostGlitchHash11(lineIndex * 3.7 + frame));
                ...
                float blockRnd = ShoostGlitchHash11(blockIndex * 9.13 + slowFrame);
                float blockGate = step(1.0 - blockShift * 0.80, ShoostGlitchHash11(blockIndex + slowFrame * 2.0 + 9.0));
                ...
                float diagonal = (ShoostGlitchHash11(slowFrame + 91.0) - 0.5) * 0.45;
                float2 dispersion = float2(dispersionPixels / max(_ScreenParams.x, 1.0), diagonal * dispersionPixels / max(_ScreenParams.y, 1.0));

                half3 glitch = ShoostGlitchSampleRgb(shiftedUv, dispersion, chromatic);
                ...
                float flash = lerp(1.0, lerp(0.55, 1.45, ShoostGlitchHash11(frame + 73.0)), flicker);
                float snow = (ShoostGlitchHash21(floor(uv * _ScreenParams.xy * 0.55) + frame) - 0.5) * noiseAmount * 0.32;
                float dropout = step(1.0 - flicker * 0.12, ShoostGlitchHash11(lineIndex * 5.0 + slowFrame * 4.0));
