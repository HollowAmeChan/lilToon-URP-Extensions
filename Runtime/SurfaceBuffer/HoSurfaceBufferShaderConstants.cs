...
namespace lilToon.URP.Extensions.SurfaceBuffer
{
    internal static class HoSurfaceBufferShaderConstants
    {
        ...
        // ---------------------------------------------------------------- 语义 lane（MSAA，规划 §0.4 / §0.3.7）

        /// <summary>语义 pass 的 LightMode：**与数值 pass 分开**（MSAA 附件不能和单采样数值图混用）。</summary>
        ...
        /// <summary>逐 sample 的 owner（16-bit IdentityId，两个字节；与数值 owner 同一个值）。</summary>
