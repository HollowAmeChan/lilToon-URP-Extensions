...
namespace lilToon.URP.Extensions.GeometryBuffer
{
    internal sealed class HoGeometryBufferRenderTargets
    {
        ...
        public void ReAllocateIfNeeded(RenderTextureDescriptor cameraTextureDescriptor, HoGeometryBufferSettings settings)
        {
            ...
            GraphicsFormat format = HoAovRenderTargets.GetHighPrecisionGraphicsFormat();
            if (format != GraphicsFormat.None)
            {
                descriptor.graphicsFormat = format;
            }

            RenderingUtils.ReAllocateIfNeeded(ref normalDepthTexture, descriptor, FilterMode.Point, TextureWrapMode.Clamp, name: "_lilHoAovNormalDepthTexture");
            RenderingUtils.ReAllocateIfNeeded(ref depthTexture, CreateDepthDescriptor(cameraTextureDescriptor, settings), FilterMode.Point, TextureWrapMode.Clamp, name: "_lilHoAovDepthTexture");
        }
        ...
        internal static RenderTextureDescriptor CreateDepthDescriptor(RenderTextureDescriptor cameraTextureDescriptor, HoGeometryBufferSettings settings)
        {
            ...
            GraphicsFormat depthFormat = HoAovRenderTargets.GetDepthStencilFormat(cameraTextureDescriptor);
