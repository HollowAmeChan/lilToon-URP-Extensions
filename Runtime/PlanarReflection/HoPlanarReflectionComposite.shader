Shader "Hidden/lilToon/URP/PlanarReflection/Composite"
{
    SubShader
    {
        ...
        Pass
        {
            ...
            // 反射用的材质数值与**遮罩**都从 AC 门面取（SB 的数值面 + OB 的覆盖率）：
            // MB 的 reflectionMaterial / surfaceColor / maskId 在这条链上全部退役。
            ...
            half4 Frag(Varyings input) : SV_Target
            {
                ...
                if (debugMode == 1)
                {
                    return half4(
                        ...
                        saturate(_HoACActive * _HoPlanarReflectionDebugInputStatus.y * _HoPlanarReflectionDebugInputStatus.w),
                ...
                // SB 的 `Color.a` 恒 1（不承载覆盖率）："有没有表面"由 owner 对齐判定（与 SSS 同一套）；
                // 遮罩里的覆盖率改由 **AC 的总覆盖率**（= OB 四层覆盖率之和）提供，不再读 MB 的 maskId。
                half surfaceMask = saturate((half)HoAC_TotalCoverage(uv)) * (HoAC_SurfaceValid(uv) ? 1.0h : 0.0h) * LilHoGeometryBufferCoverage(normalDepth);
