...
namespace lilToon.URP.Extensions.SubsurfaceScattering
{
    ...
    public sealed class HoSubsurfaceScatteringSettings
    {
        ...
        [InspectorName("源提取时机")]
        [Tooltip("捕获不透明/镂空相机颜色并提取 SSS 源颜色的时机。GeometryBuffer 需要在它之前完成。")]
        ...
        [InspectorName("强度")]
        [Tooltip("全局 SSS 合成强度。材质侧的 thickness（SB 的 Material.b）仍然控制逐像素权重。")]
        [Range(0.0f, 2.0f)]
        ...
        [InspectorName("颜色")]
        [Tooltip("用于给散射结果着色的全局颜色。材质专属配置走 per-profile 表（按 SB 的 SSS profile byte 选）。")]
        ...
        [InspectorName("材质配置")]
        [Tooltip("按 SB 的 Classification.r（Ho-SSS Profile ID）选择材质级 SSS 参数。默认 1 号为皮肤。")]
