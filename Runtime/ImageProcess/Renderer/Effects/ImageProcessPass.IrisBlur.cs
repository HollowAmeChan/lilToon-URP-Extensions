...
namespace lilToon.URP.Extensions.PostProcessing
{
    ...
    {
        private static IrisBlurParameters GetIrisBlurParameters(ImageProcessLayer layer)
        {
        ...
        {
            material.SetFloat(ImageProcessShaderConstants.RadiusId, parameters.radius * 0.01f);
            material.SetFloat(ImageProcessShaderConstants.ScreenRatioId, screenRatio);
            material.SetVector(ImageProcessShaderConstants.CenterId, new Vector4(parameters.center.x, parameters.center.y, 0.0f, 0.0f));
            material.SetFloat(ImageProcessShaderConstants.CenterSizeId, 1.0f - parameters.centerSize);
            material.SetFloat(ImageProcessShaderConstants.SmoothnessId, parameters.smoothness);
            material.SetFloat(ImageProcessShaderConstants.BlurOffsetRId, parameters.blurRadiusR * 0.01f);
            material.SetFloat(ImageProcessShaderConstants.BlurOffsetGId, parameters.blurRadiusG * 0.01f);
            material.SetFloat(ImageProcessShaderConstants.BlurOffsetBId, parameters.blurRadiusB * 0.01f);
            material.SetFloat(ImageProcessShaderConstants.DistanceId, parameters.distance * 0.01f);
            material.SetFloat(ImageProcessShaderConstants.AngleId, parameters.angleRadians);
        ...
        private void ApplyIrisBlurLayer(CommandBuffer cmd, RenderTextureDescriptor sourceDescriptor, RTHandle source, RTHandle destination, ImageProcessRuntimeLayer runtimeLayer)
        {
            ...
            RenderingUtils.ReAllocateIfNeeded(ref irisTextureA, blurDescriptor, FilterMode.Bilinear, TextureWrapMode.Clamp, name: "_lilImageProcessIrisBlurA");
            RenderingUtils.ReAllocateIfNeeded(ref irisTextureB, blurDescriptor, FilterMode.Bilinear, TextureWrapMode.Clamp, name: "_lilImageProcessIrisBlurB");
            ...
            cmd.SetGlobalTexture(ImageProcessShaderConstants.BlurredTexId, current);
            ...
            int layerIndex)
        {
            ...
            blurDesc.name = $"_lilImageProcessIrisBlur_{layerIndex}";
            ...
            TextureHandle blurredTexture = default)
        {
            using (var builder = renderGraph.AddRasterRenderPass<PassData>($"{label} Iris", out PassData passData, passProfilingSampler))
            {
                ...
                if (blurredTexture.IsValid())
                {
                    ...
                    builder.SetGlobalTextureAfterPass(blurredTexture, ImageProcessShaderConstants.BlurredTexId);
                }

                builder.SetRenderFunc(static (PassData data, RasterGraphContext context) =>
                {
                    ApplyLayerProperties(data.layer, data.material);
                    data.material.SetFloat(ImageProcessShaderConstants.RadiusId, data.radius * 0.01f);
                    data.material.SetFloat(ImageProcessShaderConstants.ScreenRatioId, data.screenRatio);
                    data.material.SetVector(ImageProcessShaderConstants.CenterId, new Vector4(data.center.x, data.center.y, 0.0f, 0.0f));
                    data.material.SetFloat(ImageProcessShaderConstants.CenterSizeId, 1.0f - data.centerSize);
                    data.material.SetFloat(ImageProcessShaderConstants.SmoothnessId, data.smoothness);
                    data.material.SetFloat(ImageProcessShaderConstants.BlurOffsetRId, data.blurOffsetR * 0.01f);
                    data.material.SetFloat(ImageProcessShaderConstants.BlurOffsetGId, data.blurOffsetG * 0.01f);
                    data.material.SetFloat(ImageProcessShaderConstants.BlurOffsetBId, data.blurOffsetB * 0.01f);
                    data.material.SetFloat(ImageProcessShaderConstants.DistanceId, data.distance * 0.01f);
                    ...
                    {
                        context.cmd.SetGlobalTexture(ImageProcessShaderConstants.BlurredTexId, data.blurredTexture);
                    }
