...
namespace lilToon.URP.Extensions.PostProcessing
{
    ...
    {
        private void ApplyRGBBlurV2Layer(CommandBuffer cmd, RenderTextureDescriptor sourceDescriptor, RTHandle source, RTHandle destination, ImageProcessRuntimeLayer runtimeLayer)
        {
            ...
            RenderingUtils.ReAllocateIfNeeded(ref rgbBlurTextureA, blurDescriptor, FilterMode.Bilinear, TextureWrapMode.Clamp, name: "_lilImageProcessRGBBlurV2A");
            RenderingUtils.ReAllocateIfNeeded(ref rgbBlurTextureB, blurDescriptor, FilterMode.Bilinear, TextureWrapMode.Clamp, name: "_lilImageProcessRGBBlurV2B");

            RTHandle current = rgbBlurTextureA;
            RTHandle next = rgbBlurTextureB;
            material.SetFloat(ImageProcessShaderConstants.RadiusId, radius);
            Blitter.BlitCameraTexture(cmd, source, current, material, 0);

            for (int i = 1; i < iterations; i++)
            {
                material.SetFloat(ImageProcessShaderConstants.RadiusId, radius + i);
        ...
        private TextureHandle RecordRGBBlurV2Layer(
            ...
            int layerIndex)
        {
            ...
            blurDesc.name = $"_lilImageProcessRGBBlurV2_{layerIndex}";
