Shader "Hidden/lilToon/URP/ImageProcess/CenterColorCorrection"
{
    SubShader
    {
        Tags
        ...
        Pass
        {
            Name "ImageProcess Center Color Correction"
            ...
            float ImageProcessCenterColorCorrectionLuminance(float3 color)
            {
                return dot(color, float3(0.2126729, 0.7151522, 0.0721750));
            }

            float3 ImageProcessCenterColorCorrectionApplyColor(float3 color, float saturation, float brightness, float contrast)
            {
                float luma = ImageProcessCenterColorCorrectionLuminance(color);
            ...
            float3 ImageProcessCenterColorCorrectionRgbToHsv(float3 color)
            {
            ...
            float3 ImageProcessCenterColorCorrectionHsvToRgb(float3 hsv)
            {
                float3 p = abs(frac(hsv.xxx + float3(0.0, 2.0 / 3.0, 1.0 / 3.0)) * 6.0 - 3.0);
                return hsv.z * lerp(float3(1.0, 1.0, 1.0), saturate(p - 1.0), hsv.y);
            }

            float3 ImageProcessCenterColorCorrectionApplyHueOffset(float3 color, float hueDegrees)
            {
                ...
                float3 hsv = ImageProcessCenterColorCorrectionRgbToHsv(max(color, 0.0));
                hsv.x = frac(hsv.x + hueOffset + 1.0);
                return ImageProcessCenterColorCorrectionHsvToRgb(hsv);
            }

            float ImageProcessCenterColorCorrectionResolveMask(float2 uv)
            {
            ...
            half4 FragCenterColorCorrection(Varyings input) : SV_Target
            {
                ...
                float mask = ImageProcessCenterColorCorrectionResolveMask(input.texcoord);
                float3 corrected = ImageProcessCenterColorCorrectionApplyColor(source.rgb, _LayerParams0.x, _LayerParams0.y, _LayerParams0.z);
                corrected = ImageProcessCenterColorCorrectionApplyHueOffset(corrected, _LayerParams2.y);
