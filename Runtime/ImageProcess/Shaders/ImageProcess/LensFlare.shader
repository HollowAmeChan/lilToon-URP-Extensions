Shader "Hidden/lilToon/URP/ImageProcess/LensFlare"
{
    SubShader
    {
        Tags
        ...
        Pass
        {
            ...
            float ImageProcessLensFlareHash(float2 p)
            {
            ...
            float2 ImageProcessLensFlareAspectDelta(float2 uv, float2 center)
            {
            ...
            float ImageProcessLensFlareSoftDisc(float2 uv, float2 center, float radius, float softness)
            {
                float dist = length(ImageProcessLensFlareAspectDelta(uv, center));
                radius = max(radius, 0.0001);
                softness = max(softness, 0.0001);
                return 1.0 - smoothstep(radius, radius + softness, dist);
            }

            float ImageProcessLensFlareGaussian(float2 uv, float2 center, float radius)
            {
                float dist = length(ImageProcessLensFlareAspectDelta(uv, center));
                radius = max(radius, 0.0001);
            ...
            float ImageProcessLensFlareRing(float2 uv, float2 center, float radius, float width)
            {
                float dist = length(ImageProcessLensFlareAspectDelta(uv, center));
            ...
            float ImageProcessLensFlareRay(float2 delta, float2 axis, float width, float lengthValue)
            {
            ...
            float3 ImageProcessLensFlareSpectralTint(float index, float chromatic)
            {
            ...
            float3 ImageProcessLensFlareLayer(float2 uv)
            {
                ...
                float2 sourceDelta = ImageProcessLensFlareAspectDelta(uv, sourceCenter);
                ...
                float core = ImageProcessLensFlareSoftDisc(uv, sourceCenter, coreSize, coreSize * 0.85 + 0.006) * 4.8;
                float corona = ImageProcessLensFlareGaussian(uv, sourceCenter, max(haloSize, coreSize + 0.001)) * 1.55;
                ...
                for (int i = 0; i < MaxLensFlareRays; i++)
                {
                    ...
                    float rayWidth = lerp(0.0035, 0.010, ImageProcessLensFlareHash(float2(i, rayCount)));
                    rays += ImageProcessLensFlareRay(sourceDelta, rayAxis, rayWidth, max(rayLength, 0.0001));
                }

                rays = rays / max((float)rayCount, 1.0) * 1.9;
                float streak = ImageProcessLensFlareRay(sourceDelta, axis, 0.0065, max(axisLength * 1.4, 0.001)) * streakIntensity * 1.35;
                float rings = ImageProcessLensFlareRing(uv, sourceCenter, max(haloSize * 0.72, coreSize + 0.02), max(haloSize * 0.045, 0.002)) * ringIntensity * 0.55;
                rings += ImageProcessLensFlareRing(uv, sourceCenter, max(haloSize * 1.15, coreSize + 0.04), max(haloSize * 0.032, 0.002)) * ringIntensity * 0.30;
                ...
                for (int g = 0; g < MaxLensFlareGhosts; g++)
                {
                    ...
                    float size = lerp(0.030, 0.095, ImageProcessLensFlareHash(float2(g, 17.0))) * lerp(0.75, 1.25, t);
                    float brightness = ghostIntensity * lerp(0.55, 0.18, t) * (1.0 - smoothstep(1.12, 1.42, length(ghostCenter - 0.5)));
                    float ghostDisc = ImageProcessLensFlareSoftDisc(uv, ghostCenter, size, size * 1.35);
                    float ghostRing = ImageProcessLensFlareRing(uv, ghostCenter, size * 1.55, max(size * 0.20, 0.002)) * ringIntensity * 0.36;
                    float3 ghostTint = tint * ImageProcessLensFlareSpectralTint((float)g, chromatic);
                    flare += ghostTint * (ghostDisc * 0.82 + ghostRing) * brightness * sourceGate;
                }

                float edgeFade = 1.0 - smoothstep(0.15, 1.35, length(uv - 0.5));
                float grain = lerp(1.0, 0.92 + ImageProcessLensFlareHash(floor(uv * _ScreenParams.xy)) * 0.16, 0.22);
                return max(flare * exposure * lerp(0.82, 1.0, edgeFade) * grain, 0.0);
            }

            float3 ImageProcessLensFlareComposite(float3 source, float3 flare, float amount, float mode)
            {
            ...
            half4 FragLensFlare(Varyings input) : SV_Target
            {
                ...
                float3 flare = ImageProcessLensFlareLayer(input.texcoord);
                if (_LayerParams3.w > 0.5)
                {
                    return half4(flare * amount, source.a);
                }

                return half4(ImageProcessLensFlareComposite(source.rgb, flare, amount, _LayerParams3.z), source.a);
            }
            ENDHLSL
        }
    }
