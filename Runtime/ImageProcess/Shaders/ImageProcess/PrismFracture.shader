Shader "Hidden/lilToon/URP/ImageProcess/PrismFracture"
{
    SubShader
    {
        Tags
        ...
        Pass
        {
            Name "ImageProcess Prism Fracture"
            ...
            float ImageProcessPrismHash11(float value)
            {
                return frac(sin(value * 127.1) * 43758.5453123);
            }

            float2 ImageProcessPrismHash21(float value)
            {
                return frac(sin(float2(value * 127.1, value * 311.7)) * 43758.5453123);
            }

            float2 ImageProcessPrismRotate(float2 value, float angle)
            {
                float s = sin(angle);
                float c = cos(angle);
            ...
            half3 ImageProcessPrismSample(float2 uv, float2 offset, float dispersion)
            {
            ...
            struct ImageProcessPrismLayerResult
            {
                half3 color;
                float weight;
            };

            ImageProcessPrismLayerResult ImageProcessPrismLayer(
                float2 uv,
                float2 local,
                float angle,
                float radial,
                ...
                float layerWeight)
            {
                ImageProcessPrismLayerResult result;
                ...
                float radialPhase = (ImageProcessPrismHash11(layerSeed + 9.1) - 0.5) * fracture * layerIndex * 0.34;
                ...
                float ringRnd = ImageProcessPrismHash11(ringSeed + 2.1);
                ...
                float ringOffset = (ImageProcessPrismHash11(ringSeed + 7.7) - 0.5) * fracture * offsetScale / ringShardCount;
                ...
                float2 rnd = ImageProcessPrismHash21(shardId) * 2.0 - 1.0;
                float rndAngle = ImageProcessPrismHash11(shardId + 5.3) * 6.2831853;
                ...
                float displacementPixels = lerp(3.0, 70.0, fracture) * displacementScale * (0.34 + ImageProcessPrismHash11(shardId + 11.0) * 0.90);
                ...
                float2 microRnd = ImageProcessPrismHash21(microShardId + 23.0) * 2.0 - 1.0;
                float shardInterior = smoothstep(0.08, 0.28, radialBreak) * smoothstep(0.08, 0.28, angularBreak);
                float microPixels = shardInterior * fracture * fracture * displacementScale * lerp(1.0, 12.0 + layerIndex * 4.0, ImageProcessPrismHash11(microShardId + 5.0));
                ...
                float localHighlightSeed = ImageProcessPrismHash11(shardId + 37.0);
                ...
                half3 prism = ImageProcessPrismSample(sampleUv, displacement * (1.0 + highlightMask * 1.25), prismDispersion);
            ...
            half4 FragPrismFracture(Varyings input) : SV_Target
            {
                ...
                float2 rotated = ImageProcessPrismRotate(local, rotation);
                float angle = atan2(rotated.y, rotated.x);
                angle = angle < 0.0 ? angle + 6.2831853 : angle;
                float radial = saturate(dist / radius);

                ImageProcessPrismLayerResult largeLayer = ImageProcessPrismLayer(input.texcoord, local, angle, radial, aspect, mask, fracture, dispersion, highlightAmount, seed, shardCount, 0.0, 0.42, 0.74, 1.35, 0.92, 0.92, 1.00, 1.00);
                ImageProcessPrismLayerResult middleLayer = ImageProcessPrismLayer(input.texcoord, local, angle, radial, aspect, mask, fracture, dispersion, highlightAmount, seed, shardCount, 1.0, 0.68, 1.06, 2.15, 0.76, 1.22, 0.82, 0.18 + fracture * 0.30);
                ImageProcessPrismLayerResult fineLayer = ImageProcessPrismLayer(input.texcoord, local, angle, radial, aspect, mask, fracture, dispersion, highlightAmount, seed, shardCount, 2.0, 0.96, 1.46, 3.20, 0.42, 1.62, 0.58, fracture * 0.22);
