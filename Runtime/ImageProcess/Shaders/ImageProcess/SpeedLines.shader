Shader "Hidden/lilToon/URP/ImageProcess/SpeedLines"
{
    SubShader
    {
        Tags
        ...
        Pass
        {
            ...
            float ImageProcessSpeedHash11(float value)
            {
                return frac(sin(value * 127.1) * 43758.5453123);
            }

            float ImageProcessSpeedLineMask(float2 local, float radial, float density, float rotation, float seed, float frame, float widthScale, float lengthScale, float coverage, float centerBlank, float lineWidth, float lengthVariation, float softness, float flicker, float brokenAmount)
            {
                ...
                float rnd = ImageProcessSpeedHash11(animatedSeed + 0.13);
                float rndWidth = ImageProcessSpeedHash11(animatedSeed + 3.71);
                float rndPresence = ImageProcessSpeedHash11(animatedSeed + 8.97);
                float rndBreak = ImageProcessSpeedHash11(animatedSeed + 15.43);
                ...
                float frameFlicker = lerp(1.0, lerp(0.72, 1.15, ImageProcessSpeedHash11(animatedSeed + 22.0)), flicker);
            ...
            half4 FragSpeedLines(Varyings input) : SV_Target
            {
                ...
                float mainMask = ImageProcessSpeedLineMask(local, radial, density, 0.0, seed, frame, 1.0, 1.0, coverage, centerBlank, lineWidth, lengthVariation, softness, flicker, brokenAmount);
                float middleMask = ImageProcessSpeedLineMask(local, radial, density * 0.58 + 5.0, 0.021, seed + 37.0, frame, 0.48, 0.72, coverage * 0.58, centerBlank, lineWidth, lengthVariation, softness, flicker, brokenAmount * 0.75);
                float fineMask = ImageProcessSpeedLineMask(local, radial, density * 1.72 + 11.0, -0.014, seed + 71.0, frame, 0.22, 0.56, coverage * 0.42, centerBlank, lineWidth, lengthVariation, softness, flicker, brokenAmount * 0.55);
