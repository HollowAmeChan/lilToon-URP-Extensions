...
### 2.1 HelpBox 预算（2026-09-22 用户反馈"太复杂啰嗦"）

**参数含义、单位、取值范围、调参建议、效果说明一律写 Tooltip**（hover 就能看到），**不要**在面板里铺成段落。

面板里只允许三类 HelpBox，且**每条最多一行**：

| 允许 | 例子 |
| --- | --- |
| 依赖 / 前置条件 | "必须排在 Ho-GeometryBuffer 之后" |
| 会**静默出错**的失败模式 | "容量不足时多出来的接收域回退普通投影" |
| 必要的**指针** | "调试模式与视图开关已移至 Ho-Xxx Volume 的「调试」分组" |
| **Volume 覆盖优先级** | "Add Override 会把该组件所有字段都设成覆盖态，所以 feature 上改不动是正常的" |

- **每个面板 HelpBox ≤ 4 条**（顶部一条 + 各 1~2 条必要提示 + 调试指针）；Volume 面板 ≤ 3 条。
  **实现限制**：Volume 的"顶部一条"可以和 §3 的调试说明行合并成一条，但**不要在顶部再补一条重复的兜底说明**。
- **§3 的调试模式说明行只算一条**：多个模式分支必须**先算好 `desc` 字符串、再调一次 `HelpBox`**，
  不要每个模式各写一次 `HelpBox(...)`（那样扫描脚本会按分支数计数，也会让预算失去意义）。
- **不再写"解释语义"的 `MessageType.None` 长句**：原来那一类（把参数原理、公式、坑讲一遍）全部搬进 Tooltip 或文档。
- **同一句话不要重复**：例如"不勾选覆盖时用 feature 兜底值"写在**分节级**一处（或直接进字段 Tooltip），不要每个字段抄一遍。
- 字段 Tooltip 写法：`效果/单位 + 取值影响 + 什么时候该调它`，一句话内解决；超长的原理、公式、踩坑记录进 `Documentation~/`。
- 分节摘要最多 3 段。
- **预算由 `.codex-research/check-ui-verbosity.ps1` 量化**：逐面板打印 HelpBox 条数与正文中文数，
  feature/组件 ≤ 4 条、Volume ≤ 3 条、正文中文 ≤ 200 字，超出即失败。改完面板跑一次。
...
| **Feature** | 运行（兜底） | 启用、单角色分辨率、阴影软边（米）、同时接收域上限、图集边长上限、深度偏移、法线偏移 |
| | 软阴影（PCSS，兜底） | 与 Volume 一一对应的六个字段 + 回退/单位说明 |
...
- 「软阴影（PCSS）」这一节用的是「内容/通道」色 `0.42, 0.72, 0.58`（与 GTAO 的「追踪」同色系），不新造颜色。它属于 CS 的"内容/质量"分节，所以按 §2 的顺序排在「运行」之后、「调试」之前，「阴影软边」是**始终生效**的基础滤波，所以放在「运行」而不是 PCSS 分组里（关掉 PCSS 它照样起作用）；PCSS 的六项才是 PCSS 专属。半径一律是米（世界单位），换「单角色分辨率」不用重调。
