...
| 约束 | 为什么 |
| --- | --- |
| ~~`阴影 → 所有效果`~~ **【已被实机校准推翻】**：**OIT 在 `Ho-ShadowCast` 之前** | 所以"效果都要吃阴影"是错的——只有排在 Ho-ShadowCast 之后的效果才吃得到附加灯 atlas；**OIT 不吃**。改成：`Ho-ShadowCast → {GTAO 之后的那批效果}`，**不含 OIT** |
...
| 实机事实 | 它推翻了什么 |
| --- | --- |
| **1. OIT 在 `Ho-ShadowCast` 之前** | 推翻"阴影 → 所有效果"：**OIT 不吃附加灯 atlas**。（URP 主光阴影仍在最前，不受影响） |
