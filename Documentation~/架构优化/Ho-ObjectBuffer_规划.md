...
- **组件上一项 `赋值方式`，默认「指定」**：
  - **指定**：重叠 = 配置错误，面板底部逐条列出（谁赢谁输），同组内**取条目顺序在前者**。这条同时把 RSUV 的写入规则（`ApplyIdentity`）与冲突面板的裁决（`ResolveAssignments`）对齐了——此前前者只按条目顺序、后者按距离，两者在同组重叠且距离不同时会互相矛盾。
  - **覆盖**：条目顺序即优先级，**排在下面的条目接管上面条目里的同一个物体**，不再报冲突。用法就是"顶上放一条『全体』（勾全角色），下面放人体 / 脸 / 前发…"——不需要反选、不需要为每个部件重复勾位。
- **覆盖不做标签继承**（这是刻意的）：覆盖条目的标签就是那些物体的全部标签，"这块归我、标签我说了算"。想让被接管的物体同时保有上面那条的位，就在覆盖条目的标签里一起勾上（例如人体那条勾「全角色 + 人体」）。理由：palette **一行 = 一个部件 = 一份标签掩码**，像素里只有 `partId`；若做继承，同一个部件里"被覆盖的物体"和"没被覆盖的物体"就需要两份不同的标签，一行装不下，只能再靠校验强制拆分——比现在更绕。
- **跨组不受影响**：仍然是"离 Renderer 更近的组胜"，且**一律报冲突**（两个组抢同一个 renderer 是真的配置问题，不管哪一边是不是覆盖模式）。
