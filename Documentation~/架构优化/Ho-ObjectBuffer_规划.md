...
- 裁决规则随之调整：**离 Renderer 更近者胜，距离相同用组 ID 定序**（原先是手填优先级）。组件上的优先级输入撤掉——默认全 0 时它本来就没起作用，真需要"上层组强行接管"再加回来。
...
#### 0.3.12 组分与选区的分工（为什么组分是写死的）

- **场景多变 ⇒ 用选区**：选区是"人自己命名的自由集合"（独立 8 bit ID 空间、材质只引用名字、可跨部件），场景里的临时需求、镜头特化、道具分组都走它。
- **角色本身就有固定语义 ⇒ 用组分**：一个东西一旦被当作"角色"来做，它需要的语义就是那几条（整角色 / 脸 / 前发 / 眼睛 / 眼透区 / 配件 / 人体）；让人给每个角色的每一块都重新起一遍名字，既啰嗦又无法跨工程对账。所以**预置一套固定组分**，作者只做"归属"这一件事。
- 一句话概括：**组分是"角色的预置选区"，选区是"场景的自由组分"**。两者最终都落成 `(ID, 覆盖率)`，差别在创建语义的成本——组分的名字不需要人创建，选区的名字必须人创建。
- 推论：① 角色特化读「组 + 组分 + 覆盖率」即可，**不依赖选区**；② 组分做成固定表还能直接当角色 AOV 的规范清单（名字恒定、跨工程可对账）；③ 组分**不是**"哪个 feature 缺一位就往里加"的地方——加一条 = 给**角色语义**加一条。

#### 0.3.13 标签位撤出面板（字段保留）

- 现在**没有任何消费端**（HLSL / C# 都不读部件与选区的 `tags`），而它那四位混了两种来源：「全角色」是**角色**语义（用组 ID 判定即可），「皮肤 / 不透明测试 / 半透明」是**表面**语义（归 SB）。
- 所以面板上的「标签」输入撤掉（部件与选区两处），字段与 palette 列**保留**（不动结构体与跨仓契约），并标注为保留位。
- 部件语义的权威路径是 R3/R4 的 `HoSemanticSchema` + 每行 lane mask。在那之前：角色语义用「组 + 角色组分」，场景/临时语义用选区。**不要**为了新 feature 往这个枚举里加位。
