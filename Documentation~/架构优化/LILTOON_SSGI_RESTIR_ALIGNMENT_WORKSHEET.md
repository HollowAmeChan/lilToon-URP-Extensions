...
| R0 | Ray candidate | `HRenderSSGI.compute:71-145` | world-space cosine ray；命中 source；candidate target=luminance(candidate)；M 包含 miss；可选 GeometryBuffer sky fallback | `部分对齐` | Raw Trace 看原始噪声；开启 sky buffer 后看低命中区域是否有稳定 fallback | 改用稳定低差异/blue-noise 序列，补 backface/depth-pyramid hit validation，确认 candidate 能量归一化 |
...
| R5 | Spatial candidate reuse | `HRestirSSGI.compute:338-440` | 独立 `Ho-SSGI Spatial Resampling` pass；world-plane Poisson 8 邻居；plane/normal/depth/Gaussian 权重；输出 spatial reservoir MRT | `部分对齐` | Frame Debugger 单独看 Spatial Resampling；Spatial reuse 开/关；边缘不能跨平面串光 | 使用稳定 Poisson buffer，补 guidance/AO scale |
| R6 | Spatial validation | `HRestirSSGI.compute:445-498` | 独立 `Ho-SSGI Spatial Validation` pass；selected-ray 8-step re-march；失败回退 temporal GI | `部分对齐` | Frame Debugger 单独看 Spatial Validation；失败时不能黑屏 | 保存 spatial guidance/occlusion，补 HTrace 第二轮 invalidity 反馈 |
