...
本文记录 `EdgeLight / 边缘光` 的新实现方向。它不再按 Shoost 的透明图片/RawImage 图层算法硬搬，而是作为 HoPost 主体数据效果实现：先显式产出角色 normal + mask，再把 HDR 边缘光合成回 camera color，让后续 ShoostStack 内部 Glow / ToonMap 继续处理它。
...
- 边缘光合成发生在 Shoost Final Stack 之前，ShoostStack 内部 Glow、ToonMap 和最终滤镜都能处理它。
- 后续 `Outline / 轮廓`、`DropShadow / 投影` 可以复用同一套主体数据 RT。

## 与 Shoost Final Stack 的关系

`EdgeLight` 不应该作为 Shoost final layer 的普通 fullscreen 滤镜执行。它可以保留 Shoost 的名称、图标和参数习惯，但运行时应归入 HoPost / subject effects RendererFeature。原因是边缘光需要主体 normal/mask；而 Shoost Final Stack 负责后续的 Glow、ToonMap、风格化滤镜和最终图层混合。

当前实现边界下，`EdgeLight / 边缘光`、`Outline / 轮廓`、`DropShadow / 投影` 已从 Shoost Final Stack 迁移到 HoPost。它们不是“普通后处理还没补 shader”，而是需要主体输入数据、subject RT、mask/stencil/depth/normal 和执行阶段的主体效果。Shoost 侧不再保留同名效果入口，enum 中对应位置只作为 removed slot 清理残留数据。

这里的 HDR 边界很重要：边缘光的 `Brightness` 应该能写出超过 1 的 HDR 能量，并在 ShoostStack 的 Glow / ToonMap 之前进入 camera color。如果把边缘光移动到 Glow / ToonMap 之后，它只能在最终显示空间加白，视觉会更硬，也不会进入 Shoost 的发光扩散链路。
...
2. `HoPost / EdgeLight`：在 Shoost Final Stack 之前运行，读取 camera color 与主体数据 RT，把 HDR 边缘光合成回 camera color。
...
- EdgeLight：`AfterRenderingPostProcessing`
- Shoost Final Stack / Glow / ToonMap：`AfterRenderingPostProcessing + 1`

EdgeLight 不应回到普通 Shoost final layer 里做 alpha 反推。需要发光扩散时，应让 HoPost 输出 HDR，再交给 ShoostStack 的 Glow 链路处理。
...
长期版本不要只从 `_CameraNormalsTexture` 直接取，因为 URP 的 normals 通常偏向 opaque/depth-normal 路径，透明 lilToon 角色未必稳定写入。我们需要自己的 subject data pass。

当前 HoPost 的第一版边缘光先以 `_CameraNormalsTexture` 作为可运行 bootstrap：RendererFeature 在存在 `EdgeLight` 图层时请求 `ScriptableRenderPassInput.Normal`，shader 用这个 normal RT 做 view-space rim，并保留外扩 mask 补偿。它运行在 Shoost Final Stack 之前，HDR 输出交给 ShoostStack 内部 Glow / ToonMap 继续处理，不依赖 URP 内置 Bloom。后续接入 subject data pass 后，应把输入切到 `_lilShoostSubjectDataTexture`，避免被普通场景 opaque normals 污染。
...
材质开关只决定“这个 renderer 能不能被 subject data pass 画进去”。不要让材质自己决定边缘光颜色、亮度或 Bloom；这些仍由 HoPost 的 EdgeLight 图层控制。
...
- `Brightness`：HDR 亮度，0-10；为 ShoostStack Glow / ToonMap 预留超过 1 的能量。
- `Contrast`：边缘硬度/对比度，0-1。
- `Angle`：方向，-180 到 180。
- `Mode`：Single / Double / Single(Sharpen) / Double(Sharpen)。
- `BlendMode`：保留 Shoost UI，但默认建议 Add 或 Screen；若使用 Normal/Overlay 等 Photoshop 模式，需要明确它们是进入 ShoostStack 前的颜色混合，不等同于 Shoost 透明 RawImage 图层。
...
3. 在 HoPost runtime 为 `HoPostProcessEffect.EdgeLight` 注册专用 shader，不走默认 no-op blit。
4. 在 HoPost editor 面板给边缘光做中文参数 UI，并固定运行在 Shoost Final Stack 之前。
5. 用 RenderDoc 验证三件事：subject data RT alpha 只包含目标角色；RGB normal 随相机变化正确；edge light pass 位于 Shoost Final Stack / Glow 之前。
...
- 不要把 edge light 做到 Shoost Glow / ToonMap 之后，除非用户明确只想要最终覆盖，不想让发光链路响应。
