...
- `ShoostPostProcessEffectRegistry.cs`：enum 到 shader name 的映射；Shoost stack 当前统一作为 `After URP Post Processing + 1` 的 final stack 执行，以便 HoPost 栈先运行。
...
架构上建议把系统拆成两条线：`Shoost Final Stack` 只做最终图层和滤镜，默认在 URP 内置后处理之后运行；`HoPostProcessing` 作为新的 RendererFeature，先承载边缘光、轮廓、投影等需要重新设计的特调效果栈。第一版 HoPost 栈统一运行在 URP 主后处理之后、Shoost Final Stack 之前，并且按用户列表顺序执行；后续如果某个效果必须被 URP Bloom 捕捉，再把它升级到更早的 subject data / lighting 阶段，而不是塞回 Shoost final stack。
...
边缘光的落地设计已单独拆到 [`ShoostEdgeLightDesign.md`](ShoostEdgeLightDesign.md)。当前已先搭出 `HoPostProcessRendererFeature` / `HoPostProcessStackVolume` / `Editor/PostProcessing/HoPost` 框架：Volume 栈可拖拽排序，包含 `EdgeLight`、`Outline`、`DropShadow` 和 `CustomMaterial` 槽位；`EdgeLight` 已按 normal rim 接入，`Outline` 第一版已按 depth + normal 屏幕空间 Sobel 接入，`DropShadow` 已按 subject mask 偏移投影接入。Runtime 侧后续仍建议新增更完整的 `SubjectData` 模块，负责分配 subject normal/mask/depth/color 并绘制 `LightMode = lilToonSubjectData` 的对象；HoPost 的具体效果再读取这些数据。Shoost stack 不再保留同名效果入口，enum 中对应位置改为空 removed slot，编辑器会清理残留槽位。
...
- `Runtime/HoPostProcessing/HoPostProcessRendererFeature.cs`：HoPost 的可排序栈框架。它读取 `HoPostProcessStackVolume`，按 Volume 列表原始顺序串行执行，不做 Shoost 那种固定排序；Compatibility path 和 RenderGraph path 都接入同一套 layer blit。执行点为 `AfterRenderingPostProcessing`，Shoost Final Stack 保持在 `AfterRenderingPostProcessing + 1`，因此边缘光的 HDR 输出会进入 ShoostStack 内部 Glow / ToonMap 等后续图层。
- `Runtime/HoPostProcessing/HoPostProcessLayer.cs` / `HoPostProcessEffect.cs`：HoPost 图层数据和效果 enum。第一版公开 `EdgeLight`、`Outline`、`DropShadow`、`DepthOfField`、`PostLighting`、`CustomMaterial`。`EdgeLight` 已接入专用 UI 与 shader，参数映射为 `Color / Intensity / Blend`、`parameters0 = Size/Brightness/Contrast/Opacity`、`parameters1 = Angle/Mode/OuterWidth/OuterAmount`；`Outline` 已接入 depth + normal Sobel shader，参数映射为 `parameters0 = Thickness/DepthWeight/NormalWeight/Threshold`、`parameters1 = Softness/DepthSensitivity/NormalSensitivity/Opacity`；`DropShadow` 已接入 subject mask 偏移投影 shader；`DepthOfField` 已接入真实深度 CoC、Bokeh/目标跟焦模式和更大的圆盘采样半径；其他槽位暂保留通用 `parameters0-5`。
- `Runtime/HoPostProcessing/Shaders/HoPost/LayerBlit.shader`：HoPost fallback shader。当前只做无效果 blit，用来保证框架、排序和自定义材质入口稳定，不把未完成的边缘光/描边/投影误做成最终效果。
- `Editor/PostProcessing/HoPost/HoPostProcessStackVolumeEditor.cs`：HoPost Volume 自定义编辑器。UI 对齐 ShoostStack：顶部使用边缘光、轮廓、投影、自定义四个图标开关增删效果，列表行只保留启用、折叠标题、强度和参数；与 Shoost stack 的固定排序不同，HoPost 的 `ReorderableList` 仍允许用户拖拽调整执行顺序。
...
- `EdgeLight / Outline / DropShadow`：已从 Shoost Final Stack 迁移到 HoPost。Shoost 侧删除同名效果入口，只保留空 removed slot 并在编辑器排序时清理；不要从最终颜色缓冲或后处理 alpha 里反推主体边界，应依赖角色 mask/stencil/depth/normal。
