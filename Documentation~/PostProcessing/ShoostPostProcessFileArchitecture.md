...
边缘光的落地设计已单独拆到 [`ShoostEdgeLightDesign.md`](ShoostEdgeLightDesign.md)。当前已先搭出 `HoPostProcessRendererFeature` / `HoPostProcessStackVolume` / `Editor/PostProcessing/HoPost` 框架：Volume 栈可拖拽排序，包含 `EdgeLight`、`Outline`、`DropShadow` 和 `CustomMaterial` 槽位，默认 fallback shader 只做 no-op blit，避免未接入主体数据时误导画面。Runtime 侧后续仍建议新增 `SubjectData` 模块，负责分配 subject normal/mask/depth/color 并绘制 `LightMode = lilToonSubjectData` 的对象；HoPost 的具体效果再读取这些数据。Shoost stack 可以保留同名入口作为参数参考或兼容层，但不应继续承担主体数据效果的实际执行。
...
- `Runtime/HoPostProcessing/HoPostProcessRendererFeature.cs`：HoPost 的可排序栈框架。它读取 `HoPostProcessStackVolume`，按 Volume 列表原始顺序串行执行，不做 Shoost 那种固定排序；Compatibility path 和 RenderGraph path 都接入同一套 layer blit。执行点为 `AfterRenderingPostProcessing`，Shoost Final Stack 统一后移到 `AfterRenderingPostProcessing + 1`，因此 HoPost 总是在 URP 主后处理之后、Shoost 之前。
- `Runtime/HoPostProcessing/HoPostProcessLayer.cs` / `HoPostProcessEffect.cs`：HoPost 图层数据和效果 enum。第一版公开 `EdgeLight`、`Outline`、`DropShadow`、`CustomMaterial`，参数先保留通用 `Color / Intensity / Blend / Texture / parameters0-5`，等待具体算法接入后再拆专用 UI。
...
- `Editor/PostProcessing/HoPost/HoPostProcessStackVolumeEditor.cs`：HoPost Volume 自定义编辑器。使用可拖拽 `ReorderableList`，顶部提供边缘光、轮廓、投影、自定义四个添加按钮；添加后自动展开，不再像 Shoost stack 一样按固定顺序强制重排。
