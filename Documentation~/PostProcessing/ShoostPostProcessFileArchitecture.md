...
- `Runtime/HoPostProcessing/HoPostProcessLayer.cs` / `HoPostProcessEffect.cs`：HoPost 图层数据和效果 enum。第一版公开 `EdgeLight`、`Outline`、`DropShadow`、`CustomMaterial`。`EdgeLight` 已接入专用 UI 与 shader，参数映射为 `Color / Intensity / Blend`、`parameters0 = Size/Brightness/Contrast/Opacity`、`parameters1 = Angle/Mode/OuterWidth/OuterAmount`；`Outline` 已接入 depth + normal Sobel shader，参数映射为 `parameters0 = Thickness/DepthWeight/NormalWeight/Threshold`、`parameters1 = Softness/DepthSensitivity/NormalSensitivity/Opacity`；`DropShadow` 已接入 subject mask 偏移投影 shader；其他槽位暂保留通用 `parameters0-5`。
