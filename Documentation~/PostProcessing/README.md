...
- **效果浏览器**（两个编辑器共用）：顶栏搜索（只按中文标签与枚举名匹配，样式开关在最左、输入框占满其余宽度）+ 左侧可翻页图标侧栏（同一个侧栏宽度：默认纯图标 3×10 = 30/页，可切图标+名字 1×10 = 10/页，翻页箭头不画按钮底）+ 右侧原有图层列表（搜索只高亮命中行，不过滤不重排）；图标右键有添加/移除/重置菜单，图层行有 `×` 移除。见 `EffectBrowser.md`。
