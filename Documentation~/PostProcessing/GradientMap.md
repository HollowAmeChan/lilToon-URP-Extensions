...
标题行还带预设按钮：`默认`（重置成黑白渐变）+ 5 组共 16 个 look。渐变字段下面有一条 64 段的色标条预览，它用的是**同一次烘焙**与同样的采样规则（含插值空间、透明度键与 Fixed 台阶），所以预览与渲染一致。
...
- **没有 Unity 实机画面**：C# 烘焙核心是**真的执行过**的（dotnet 工程 11 项检查），但"参数 → 画面"的其余部分（贴图绑定、RenderGraph 路径、Inspector 布局）只做了静态编译与结构检查。需要在实机确认：
  ...
  3. 面板控件顺序：色标 → 预览条 → 输入 → 黑场 → 白场 → 插值空间 → 色阶数 → 反转 → 抖动 → 混合模式；
  4. 预览条与渲染一致（拖动键、切 `Fixed`、改插值空间时）；
  5. 5 组 16 个预设的观感，以及"海报平涂"的 6 级平涂在 8-bit 目标上是否需要补抖动；
  6. `Fixed` 模式的台阶位置与 Unity 编辑器渐变条是否一致；
  7. 运行时 ramp 贴图在 RenderGraph 路径下正常绑定（Material 绑定路径与兼容路径共用 `ApplyLayerRamp`），以及切换图层效果后贴图会被回收。
