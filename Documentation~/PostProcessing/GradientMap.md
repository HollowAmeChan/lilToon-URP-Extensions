...
- `Images/GradientMap/looks.png`：11 个预设套在合成测试图上的接触表，每格下方是该预设的色标条。
...
标题行还带预设按钮：`默认`（重置成黑白线性色标）+ 4 组共 11 个 look。
...
| 组 | 名称 | 做了什么 | 来源/可信度 |
| --- | --- | --- | --- |
| 双色调 | 棕褐 | 暗部暖褐、亮部奶油，Oklab 插值 | 配方（本文档作者按常规棕褐配色手写） |
| 双色调 | 冷夜 | 压暗偏蓝 + 冷青亮部 | 配方（"白天拍夜戏靠压暗+偏蓝"是通行做法） |
| 双色调 | 青橙 | 暗部青、亮部橙 | 配方（商业片常见的冷暖对撞） |
| 胶片 | 高对比黑白 | 印片曲线式 S 形灰阶 | 配方 |
| 胶片 | 褪色 | 黑位抬到深灰、白位压到米白 | 配方（"褪色胶片 / Faded Film"一类） |
| 胶片 | 漂白旁路 | 灰阶色标 + `明度` 混合，只改影调不改色彩 | 配方（银盐保留式的做法） |
| 胶片 | 负片 | 反转 + 灰阶 | 配方 |
| 风格 | 日落 | 紫罗兰→洋红→橙金 | 配方 |
| 风格 | 赛博霓虹 | 近黑紫→紫→洋红→青 | 配方 |
| 风格 | 海报平涂 | 青橙色标 + 色阶数 6 | 配方 |
| 风格 | 红外假色 | 以红通道索引（输入 = 红） | 配方；**是风格化近似，不是红外胶片的色彩科学** |

> 这一批 look 的颜色是手写配方（不是从某个官方调色板采样）。`.codex-research/gradient_map_research/` 里在补"从已发布调色板（matplotlib 的 inferno/viridis/turbo、热像仪的 ironbow 等）采样 4 个色标"的那一组 look，落地后会在本节标明每个数字的出处。
...
- **ramp 数学**：`.codex-research/gradient_map_sim/gradient_map_check.js`（Node，14 项）在 shader 的 1:1 复刻上验证：默认色标在灰阶上是恒等（1.1e-16）、输入窗口的截断/重映射、反转镜像、色阶数 6 恰好 6 级且首末为 0/1、零宽色标是硬切、退化窗口是阈值、alpha=0 色标不动画面、线性光与 Oklab 中点与**独立实现**的差（0 与 2.0e-10）、Luminosity 混合把色标的亮度搬到原图上、11 个 look 都能被渲染且色标合法；负对照（扰动 Oklab 矩阵）会让 Oklab 检查失败（1.2e-3）。
...
1. 改 `.codex-research/gradient_map_sim/looks.js`（颜色可以写 sRGB hex）。
...
3. `node verify_csharp.js`（漂移 + 逐字段比对）与 `node gradient_map_check.js`（其中一项就是"所有 look 都合法且能被渲染"）。
...
- **没有 Unity 实机画面**：HLSL 只做了编译验证，C# 只做了静态编译，"参数 → 画面"的结论全部来自 JS 复刻。需要在实机确认的清单：
  ...
  5. 4 组 11 个预设的观感，以及「海报平涂」的 6 级平涂是否需要在 8-bit 目标上补抖动。
