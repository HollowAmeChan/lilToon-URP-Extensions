...
- 已实现为独立效果 `ImageProcessEffect.GradientMap`「渐变映射」：色标用 Unity 原生 `Gradient`（≤8 颜色键 + ≤8 透明度键，Blend/Fixed）并在运行时烘焙成 1×256 的 ramp 贴图（shader 只采样一次），输入选择（亮度/红/绿/蓝/最大值/平均值/饱和度）、黑场白场窗口、反转、色阶数（平涂）、显示空间/线性光/Oklab 烘焙空间、输出抖动，另有 5 组 16 个 look（其中 5 个的颜色采样自已发布色表：matplotlib copper/inferno/viridis、Google turbo、FLIR 风格 ironbow）。
