...
- `专用调度`：仍然是 Shoost 后处理语义，但需要多 RT、历史 buffer 或 profile 组合。典型项是模糊、运动轨迹、帧率限制、Tube、胶片。
- `主体数据效果`：依赖角色边界、alpha、depth 或 normal，不按 Shoost 透明图片源硬搬。典型项是边缘光、轮廓、投影，也包括可能需要 mask 的发光和光照。
- `场景/相机控制`：更像 Shoost 场景功能，不应伪装成单个颜色后处理。典型项是透明背景、粒子、天气、摄像头切换器。
...
- 另外保留一排“旧实现”图标，只放已经有实际 shader 或明确兼容价值、但还没归并到 Shoost 用户侧入口的实现；纯占位和调试逃生口不显示。
- 不再单独显示“锐化（后）”。用户侧只保留 Shoost 的“锐化”，如果需要后置执行，就在高级设置里把插入位置改到 After URP Post Processing。
