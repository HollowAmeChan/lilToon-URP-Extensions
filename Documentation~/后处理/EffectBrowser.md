...
- 命中数**不占版面**：悬停页码（`1/2`）显示「侧栏命中 n · 列表高亮 m」；m = 0 时写"列表里没有"——
  顺带回答"我到底加没加过它"。
...
| `EffectBrowserSearch.cs` | **纯 C#、无 UnityEngine**：`Normalize` / `Matches` / `Filter` / `PageSize(30,10)` / `Columns` / `Rows` / `PageCount` / `ClampPage` / `PageRange` / `FormatPageLabel` / `FormatCounts` |
...
| `EffectBrowserView.cs` | 画搜索栏（样式开关 + 输入框 + 清空）、侧栏（无底翻页箭头 + 页码 + 图标网格）、右键菜单、窄宽度回退；`LoadIcon`（唯一图标入口，带缓存，先包路径后 `Assets/Editor/ImageProcessIcons`）；`CurrentQuery` 供列表高亮读取 |
