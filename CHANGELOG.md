...
- `Ho-ObjectBuffer`：组件新增**「赋值方式」**（默认**指定**）——把"一个物体被多个部件条目命中"这件事显式化（规划 0.3.15）。
  - **指定**：重叠 = 配置错误，面板底部逐条列出谁赢谁输，同组内**取条目顺序在前者**。顺带把两处裁决对齐了：RSUV 的写入（`ApplyIdentity`）此前只按条目顺序、冲突面板（`ResolveAssignments`）按距离，同组重叠且距离不同时两者会互相矛盾，现在统一为条目顺序。
  - **覆盖**：条目顺序即优先级，**排在下面的条目接管上面条目里的同一个物体**，不再报冲突 —— "顶上放一条『全体』（勾全角色），下面放人体 / 脸 / 前发…"这种用法不再需要反选没归属的物体、也不用给每个部件重复勾位。
  - **覆盖不做标签继承**（刻意）：palette 一行 = 一个部件 = 一份标签掩码，像素里只有 `partId`；做继承会让"同一部件里被覆盖与没被覆盖的物体"需要两份标签、一行装不下，只能再靠校验强制拆部件。所以覆盖条目的标签就是那些物体的全部标签（想让它们同时保有上一条的位，就在覆盖条目里一起勾）。
